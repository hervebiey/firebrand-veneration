export const VictoryIsMyName = {
	id: "victory-is-my-name",
	session: 1,
	order: 4,
	title: "Victory Is My Name",
	artist: "Sinach & Israel Houghton",
	lead: "Lynn & Raissa",
	language: "English",
	key: "D♭",
	melody: "Alto, Tenor (Bridge), Soprano (Bridge, On Switch)",
	length: {minutes: 7},
	youtube: "9kh08vUGhzs",
	chordify: "https://chordify.net/chords/victory-is-my-name-sinach",
	notes: "With Choreography",
	audio: {
		src: "/songs/victory-is-my-name.mp3",
		type: "audio/mpeg",
	},
	soprano: {
		src: "/background/victory-is-my-name.m4a",
		type: "audio/mpeg",
	},
	alto: {
		src: "/background/victory-is-my-name.m4a",
		type: "audio/mpeg",
	},
	tenor: {
		src: "/background/victory-is-my-name.m4a",
		type: "audio/mpeg",
	},
	lyrics: [
		{
			section: "Instrumental Interlude",
			notes: "2x Bars",
		},
		{
			section: "Verse 1",
			notes: "1x Solo, 1x Unison & Harmony",
			text: `The Lord is my Shepherd, I shall not want [Unison]
				The Lord has provided more than enough [Unison]
				No matter where I go, oh oh oh [Harmony]
				Victory is my name [Harmony]`
		},
		{
			section: "Chorus 1",
			notes: "1x Harmony",
			text: `My praise is strong, joy overflowing
				More than a song, I'm celebrating
				In the midst of my enemies, I praise
				In the midst of my enemies, I praise, I praise`
		},
		{
			section: "Chorus 2",
			notes: "1x Harmony",
			text: `I'm gonna put on my dancing shoes
				There's no way that I can lose
				I'll be the last one standing
				I'll be the last one standing`
		},
		{
			section: "Verse 2",
			notes: "1x Solo, 1x Harmony",
			text: `The Lord is Tower, a Shield for me
				With all of the power I'll ever need
				No matter where I go, oh oh oh
				Victory, victory is my name`
		},
		{
			section: "Chorus 1",
			notes: "1x Harmony",
			text: `My praise is strong, joy overflowing
				More than a song, I'm celebrating
				In the midst of my enemies, I praise
				In the midst of my enemies, I praise, I praise`
		},
		{
			section: "Chorus 2",
			notes: "1x Harmony",
			text: `I'm gonna put on my dancing shoes
				There's no way that I can lose
				I'll be the last one standing
				I'll be the last one standing`
		},
		{
			section: "Bridge 1",
			notes: "2x Unison, 2x Harmony: Tenor Melody",
			text: `My name, my name
				Victory is my name my name`
		},
		{
			section: "Bridge 2",
			notes: "2x Solo",
			text: `Goodness and mercy follows me
				All my days, all my days`
		},
		{
			section: "Bridge 3",
			notes: "2x Solo",
			text: `Up ahead and behind
				On the left and the right`
		},
		{
			section: "Bridge 2",
			notes: "2x Harmony: Tenor Melody",
			text: `Goodness and mercy follows me
				All my days, all my days`
		},
		{
			section: "Bridge 3",
			notes: "2x Harmony: Tenor Melody",
			text: `Up ahead and behind
				On the left and the right`
		},
		{
			section: "Refrain",
			notes: "1x Solo",
			text: `In the midst of my enemies, I praise
				In the midst of my enemies, I praise, I praise`
		},
		{
			section: "Chorus 2",
			notes: "2x Harmony",
			text: `I'm gonna put on my dancing shoes
				There's no way that I can lose
				I'll be the last one standing
				I'll be the last one standing`
		},
		{
			section: "Bridge 1",
			notes: "2x Unison",
			text: `My name, my name
				Victory is my name my name`
		},
		{
			section: "Bridge 1\'",
			notes: "2x Harmony: Tenor Melody",
			text: `My name, my name
				Favor is my name my name
				My name, my name
				Favor is my name my name
				
				My name, my name
				Miracle is my name my name
				My name, my name
				Blessing is my name my name
				My name, my name
				Blessing is my name my name
				My name, my name
				Wealth is my name my name
				My name, my name
				Wealth is my name my name
				My name, my name
				Blessing is my name my name
				`
		},
		{
			section: "Bridge 3",
			notes: "1x Silent, 1x Solo, 5x Harmony: Tenor Melody",
			text: `Up ahead and behind
				On the left and the right`
		},
		{
			section: "Bridge 3",
			notes: "4x Silent, 4x Harmony: Soprano Melody",
			text: `Up ahead and behind
				On the left and the right`
		},
		{
			section: "Chorus 1",
			notes: "1x Harmony",
			text: `My praise is strong, joy overflowing
				More than a song, I'm celebrating
				In the midst of my enemies, I praise
				In the midst of my enemies, I praise, I praise`
		},
		{
			section: "Chorus 2",
			notes: "1x Harmony",
			text: `I'm gonna put on my dancing shoes
				There's no way that I can lose
				I'll be the last one standing
				I'll be the last one standing`
		},
	]
}